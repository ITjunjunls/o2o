...
CREATE TABLE `tb_local_auth`  (
  ...
  UNIQUE INDEX `keyuk_local_profile`(`username`) USING BTREE,
...
CREATE TABLE `tb_product_img`  (
  ...
  `img_desc` varchar(2000) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `priority` int(2) NULL DEFAULT 0,
  `create_time` datetime(0) NULL DEFAULT NULL,
...
CREATE TABLE `tb_shop`  (
  ...
  INDEX `fk_shop_cate`(`shop_category_id`) USING BTREE,
  CONSTRAINT `fk_shop_area` FOREIGN KEY (`area_id`) REFERENCES `tb_area` (`area_id`) ON DELETE RESTRICT ON UPDATE RESTRICT,
  CONSTRAINT `fk_shop_cate` FOREIGN KEY (`shop_category_id`) REFERENCES `tb_shop_category` (`shop_category_id`) ON DELETE RESTRICT ON UPDATE RESTRICT,
